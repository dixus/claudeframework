...
9. Validation criteria check: if the spec has a "Validation criteria" section, confirm each criterion is still met after the fixes — note any that are now failing as new issues to address.
10. Self-improvement gate: for each fixed issue, ask whether a rule added to CLAUDE.md or a context file would prevent recurrence. If yes, note it in the report — flag it as a suggested rule addition for the team to review.
11. Report:
    - Which issues were fixed
    - Which (if any) were intentionally skipped and why
    - Whether the spec was updated and why
    - Any suggested CLAUDE.md rules to prevent recurrence (from step 10)
    - Final verify status for each command
12. Remind the user to run `/2_review` again in a fresh session to confirm all issues are resolved and no new ones were introduced
