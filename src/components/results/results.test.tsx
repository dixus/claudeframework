...
const baseResult: AssessmentResult = {
  ...
  rawLevel: 2,
  gated: false,
  level: { level: 2, label: 'AI-Enabled', monthsTo100M: 24, arrPerEmployee: '€600K' },
  dimensions: [
    { key: 'strategy',     label: 'Strategy',     weight: 0.20, score: 40.0 },
    { key: 'architecture', label: 'Architecture', weight: 0.15, score: 75.0 },
    { key: 'workflow',     label: 'Workflow',      weight: 0.25, score: 80.0 },
    { key: 'data',         label: 'Data',          weight: 0.15, score: 70.0 },
    { key: 'talent',       label: 'Talent',        weight: 0.15, score: 65.0 },
    { key: 'adoption',     label: 'Adoption',      weight: 0.10, score: 60.0 },
  ],
  bottleneck: {
    dimension: 'strategy',
    score: 40.0,
...
describe('ScoreCard gating', () => {
  it('renders a gating notice when gated=true', () => {
    ...
    expect(screen.getByText(/gating conditions were not met/i)).toBeInTheDocument()
  })

  it('does not render a gating notice when gated=false', () => {
    render(<ScoreCard result={baseResult} />)
    expect(screen.queryByText(/gating conditions were not met/i)).not.toBeInTheDocument()
  })
})
...
describe('BottleneckPanel', () => {
  it('renders the bottleneck dimension label, gap, and all 3 actions', () => {
    ...
    expect(screen.getByText(/30\.0 points to the 70 target/i)).toBeInTheDocument()
    expect(screen.getByText(/Define an explicit AI strategy/i)).toBeInTheDocument()
    expect(screen.getByText(/Establish an AI governance board/i)).toBeInTheDocument()
    expect(screen.getByText(/Allocate a dedicated AI budget/i)).toBeInTheDocument()
  })
})
...
describe('ResultsPage', () => {
  it('clicking Start Over calls reset() and returns to step 0', async () => {
    useAssessmentStore.setState({ result: baseResult, step: 9 })
    render(<ResultsPage />)
